...
CATResult CATVideoCapture::Init(CATWND wnd, const CATRect& rect,
                                CATCaptureCB callback, void* context) {
  Cleanup();
  fTargetRect = rect;
  fWnd = wnd;
  fCallback = callback;
...
CATResult CATVideoCapture::Start() {
  if (!fInternal->fGraph) return CAT_ERR_NOT_INITIALIZED;
  ...
  /* Source file test
          {
                  ...
                  if (S_OK == fInternal->fGraph->RenderFile((const
     wchar_t*)sourceFile,0)) result = CAT_SUCCESS;
          }
  */
  if (CATFAILED(result = InitCapture())) return result;

  if (FAILED(fInternal->fControl->Run())) result = CAT_ERR_CAPTURE_CREATE;
...
CATResult CATVideoCapture::CreateGraph() {
  ...
  CATResult res = fInternal->fAllocPres->Init(
      fWnd, fTargetRect.Width(), fTargetRect.Width(), fCallback, fContext);
  if (CATFAILED(res)) return res;

  // Create graphbuilder and filtergraph
  // goto or throw works too.  just bail on any error and cleanup anything we
  // created.
  for (;;) {
    HRESULT hr;

    if (FAILED(hr = CoCreateInstance(CLSID_FilterGraph, 0, CLSCTX_INPROC_SERVER,
                                     IID_IGraphBuilder,
                                     (void**)&fInternal->fGraph)))
      break;

    if (FAILED(hr = CoCreateInstance(
                   CLSID_CaptureGraphBuilder2, 0, CLSCTX_INPROC_SERVER,
                   IID_ICaptureGraphBuilder2, (void**)&fInternal->fCapGraph)))
      break;

    fInternal->fCapGraph->SetFiltergraph(fInternal->fGraph);

    // Get control interface (start/stop/etc)
    if (FAILED(hr = fInternal->fGraph->QueryInterface(
                   IID_IMediaControl, (void**)&fInternal->fControl)))
      break;
    ...
    if (FAILED(hr = fInternal->fRenderer->QueryInterface(
                   IID_IVMRFilterConfig9, (void**)&fInternal->fVMR9Config)))
      break;

    fInternal->fVMR9Config->SetNumberOfStreams(1);

    if (FAILED(hr =
                   fInternal->fGraph->AddFilter(fInternal->fRenderer, L"VMR9")))
      break;

    if (FAILED(
            hr = fInternal->fVMR9Config->SetRenderingMode(VMR9Mode_Renderless)))
      break;

    // Hook in our custom allocator/presenter
    if (FAILED(hr = fInternal->fRenderer->QueryInterface(
                   IID_IVMRSurfaceAllocatorNotify9,
                   (void**)&fInternal->fVMR9Notify)))
      break;

    hr = fInternal->fVMR9Notify->SetD3DDevice(
        fInternal->fAllocPres->GetDevice(),
        fInternal->fAllocPres->GetMonitor());
    hr = fInternal->fVMR9Notify->AdviseSurfaceAllocator(0,
                                                        fInternal->fAllocPres);
...
CATResult CATVideoCapture::InitCapture() {
  ...
  if (FAILED(hr = devEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory,
                                                 &classEnum, 0)) ||
      (classEnum == 0))
  ...
  while (S_OK == (hr = classEnum->Next(1, &moniker, 0))) {
    ...
    if (FAILED(moniker->BindToStorage(0, 0, IID_IPropertyBag,
                                      (void**)(&propBag)))) {
      moniker->Release();
      continue;
    }
    ...
    if (SUCCEEDED(hr)) {
      ...
      int strLength =
          WideCharToMultiByte(CP_ACP, 0, devName.bstrVal, -1, 0, 0, 0, 0);
      WideCharToMultiByte(CP_ACP, 0, devName.bstrVal, -1,
                          fDevName.GetAsciiBuffer(128), strLength, 0, 0);
      fDevName.ReleaseBuffer();
      CATTRACE(fDevName);

      // OK, got a device.  Let's just take the first one, it's late.
      hr = moniker->BindToObject(0, 0, IID_IBaseFilter,
                                 (void**)&fInternal->fCaptureFilter);
      if (SUCCEEDED(hr)) {
        hr = this->fInternal->fGraph->AddFilter(fInternal->fCaptureFilter,
                                                L"Capture");
        if (SUCCEEDED(hr)) {
          hr = fInternal->fCapGraph->RenderStream(
              &PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video,
              fInternal->fCaptureFilter, NULL, fInternal->fRenderer);
          break;
        }
      }
    }
...
CATResult CATVideoCapture::LockImage(CATUInt32 wait) {
  if (!fInternal->fAllocPres) return CAT_ERROR;

  return fInternal->fAllocPres->LockImage(wait);
}

void CATVideoCapture::ReleaseImage() {
  if (fInternal->fAllocPres) fInternal->fAllocPres->ReleaseImage();
}
